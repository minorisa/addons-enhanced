            ...
            this.$pager
                .on('click', 'a[data-pager-action]', function () {
                    ...
                    .click(function (e) {
                        ...
                        // JP: 20160121
                        var dictLimitStops = "200,500,2000";
                        var dictHideUnlimited = true;                        		
                        var xopts = '<option value="80">80</option>';
                        var Mopts = new instance.web.Model('ir.values');
                        Mopts.call('get_defaults_dict', ['list.view.limit.options'], undefined, {shadow:true})
                        	.fail(function(dict){
                                dictLimitStops = "200,500,2000";
                                dictHideUnlimited = true;                        		
                        	})
                        	.done(function(dict){
                                dictLimitStops = dict['limit_stops'] || "200,500,2000";
                                dictHideUnlimited = dict['hide_unlimited'];                        		
                        	})
                        	.then(function(dict){
                                dictLimitStops = dict['limit_stops'] || "200,500,2000";
                                dictHideUnlimited = dict['hide_unlimited'];                        		
                                var arropts = dictLimitStops.split(",");
                                for (var i = 0, len = arropts.length; i < len; i++){
                                	xopts = xopts + '<option value="' + arropts[i] + '">' + arropts[i] + '</option>';
                                }
                                if (!dictHideUnlimited) {
                                	xopts = xopts + '<option value="NaN">' + _t("Unlimited") + '</option>';
                                }
                                var $select = $('<select>')
                                .appendTo($this.empty())
